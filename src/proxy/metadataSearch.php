...
class CSWSearch
{
	...
	function sendRequest()
	{
		...
		$p->_url=$this->_sendServers[$this->_currentServerCount];
	...
	function manipulateResponse()
	{
		...
		if ($dom_xml->getElementsByTagName("SearchResults")->length>0)
		{
			...
			foreach ($dom_xml_results as $key=>$value)
			{
				...
				{
					case "3":
						$fileIdentifier=$value->getElementsByTagName("identifier")->item(0)->nodeValue;
						$codeList=$value->getElementsByTagName("type")->item(0)->nodeValue;
						$title=$value->getElementsByTagName("title")->item(0)->nodeValue;
						
						$LowerCorner=$value->getElementsByTagName("LowerCorner")->item(0)->nodeValue;
						$LowerCornerArr=explode(" ",$LowerCorner);
						
						$UpperCorner=$value->getElementsByTagName("UpperCorner")->item(0)->nodeValue;
						...
						$abstract=$value->getElementsByTagName("abstract")->item(0)->nodeValue;
					
					break;
					
					default:
					
						$fileIdentifier=$value->getElementsByTagName("fileIdentifier")->item(0)->getElementsByTagName("CharacterString")->item(0)->nodeValue;
						$codeList=$value->getElementsByTagName("MD_ScopeCode")->item(0)->getAttribute("codeListValue");
						$title=$value->getElementsByTagName("CI_Citation")->item(0)->getElementsByTagName("title")->item(0)->nodeValue;
						$bboxWest=$value->getElementsByTagName("EX_GeographicBoundingBox")->item(0)->getElementsByTagName("westBoundLongitude")->item(0)->nodeValue;
						$bboxSouth=$value->getElementsByTagName("EX_GeographicBoundingBox")->item(0)->getElementsByTagName("southBoundLatitude")->item(0)->nodeValue;
						$bboxEast=$value->getElementsByTagName("EX_GeographicBoundingBox")->item(0)->getElementsByTagName("eastBoundLongitude")->item(0)->nodeValue;
						$bboxNorth=$value->getElementsByTagName("EX_GeographicBoundingBox")->item(0)->getElementsByTagName("northBoundLatitude")->item(0)->nodeValue;
						$abstract=$value->getElementsByTagName("abstract")->item(0)->getElementsByTagName("CharacterString")->item(0)->nodeValue;
				...
				if (file_exists($this->pathToImages."/metadata_".strtolower(trim($codeList)).".png"))
				{
				...
				$this->_arrayResponse[]=array(
					...
					"format"=>strtolower(trim($codeList)),
					"title"=>trim($title),
					"icon"=>$icon,
					"west"=>trim($bboxWest),
