...
class getRecordsById
{
	...
	function getRecordsById($data)
	{
		...
		$this->viewAsXMLImg="<a href=\"metadataGetRecordsByIdXML.php?recordId=".$this->_recordId."&server=".$this->_serverUrl."\" target=\"_new\"><img src=\"".$this->pathToImages."xml.png\" alt=\"".$this->headersArr["viewasxml"]."\" style=\"vertical-align: middle\"> ".$this->headersArr["viewasxml"]."</a>";
	...
	function sendRequest()
	{
		...
		$p->_url=$this->_serverUrl;

		$p->_postData=$this->_sendRequest;

		$p->get();

		$this->xmlResponse=$p->o_response;

		$this->xmlResponse;

		$this->buildGMDHTML(NULL,NULL,NULL,NULL,NULL);
	...
	function datasetGetLayerName($url,$identifiers)
	{
		...
		foreach($layers as $key=>$layer)
		{
			...
			{
				$identity=$v->nodeValue;

				if(in_array($identity,$identifiers))
				{
					$layerName=$layer->getElementsByTagName("Name")->item(0)->nodeValue;
	...
	function queryXml($dom_xml,$path,$attribute="")
	{
		...
		foreach ($entries as $entry)
		{
			if(empty($attribute))
			{
				$output[$entry->localName][]=trim($entry->nodeValue);
			}
			else
			{

				$output[$entry->localName][]=trim($entry->getAttribute($attribute));
			}
		}
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<style>
			...
			.md_header{
				background:#256E21 url(<?php echo $gR->pathToImages;?>metadata_header.png);
				color:#FFF278;
				font-size:22px;
				padding:2px;
				padding-left:10px;
		...
		<link rel="stylesheet" href="../../tools/jquery/css/smoothness/jquery-ui.css">
		<script type="text/javascript" src="../../tools/jquery/js/jquery-1.9.1.js"></script>
		<script type="text/javascript" src="../../tools/jquery/js/jquery-ui.js"></script>
		<script src="../../tools/proj4js/lib/proj4js.js"></script>
	...
	<body >
		<div id="md_header" class="md_header">
			...
			<?php
				...
				$ext=$gR->buildGMDHTML("//".$md_mi_srv."/gmd:hierarchyLevel/gmd:MD_ScopeCode","codeListValue","",true,NULL);

				if (file_exists($gR->pathToImages."metadata_".$ext.".png"))
				{
					echo "<img src=\"".$gR->pathToImages."metadata_".$ext.".png\">";
				}

			?>

			<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:identificationInfo/".$md_srv."/gmd:citation/gmd:CI_Citation/gmd:title","","",true,NULL);?>
		...
		<div id="md_container" class="md_div">
			...
			<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:fileIdentifier",NULL,NULL,NULL,NULL);?>
			<?php
				$flag=strtolower($gR->buildGMDHTML("//".$md_mi_srv."/gmd:language/gmd:LanguageCode","codeListValue","",true,NULL));
				...
				{
					if (file_exists($gR->pathToImages."flags/".$value.".png"))
					{
						echo "<li>".$value." <img src=\"".$gR->pathToImages."flags/".$flag.".png\" style=\"vertical-align: middle\" alt=\"".$flag."\"></li>";
					}
					else
					{
						echo "<li>".$value."</li>";
			...
			<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:characterSet/gmd:MD_CharacterSetCode","codeListValue",NULL,NULL,NULL);?>
			<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:hierarchyLevel/gmd:MD_ScopeCode","codeListValue",NULL,NULL,NULL);?>
			<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:identificationInfo/".$md_srv."/srv:serviceType",NULL,NULL,NULL,NULL);?>
			</div>

			<h3><b><?php echo $gR->headersArr["header_metadata"];?></b></h3>
			<div id="md_contact" class="md_div md_inner_div">
				<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:contact/gmd:CI_ResponsibleParty/gmd:organisationName",NULL,NULL,NULL,NULL);?>
				<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:contact/gmd:CI_ResponsibleParty/gmd:contactInfo/gmd:CI_Contact/gmd:phone/gmd:CI_Telephone/gmd:voice",NULL,NULL,NULL,NULL);?>
				<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:contact/gmd:CI_ResponsibleParty/gmd:contactInfo/gmd:CI_Contact/gmd:phone/gmd:CI_Telephone/gmd:facsimile",NULL,NULL,NULL,NULL);?>
				<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:contact/gmd:CI_ResponsibleParty/gmd:contactInfo/gmd:CI_Contact/gmd:address/gmd:CI_Address/gmd:electronicMailAddress",NULL,NULL,NULL,NULL);?>
				<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:contact/gmd:CI_ResponsibleParty/gmd:role/gmd:CI_RoleCode","codeListValue",NULL,NULL,NULL);?>
			</div>

			<h3><b><?php echo $gR->headersArr["header_identification"];?></b></h3 >
			<div id="md_identification" class="md_div md_inner_div" >
				<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:identificationInfo/".$md_srv."/gmd:citation/gmd:CI_Citation/gmd:title",NULL,NULL,NULL,NULL);?>
				<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:identificationInfo/".$md_srv."/gmd:citation/gmd:CI_Citation/gmd:alternateTitle",NULL,NULL,NULL,NULL);?>

				<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:identificationInfo/".$md_srv."/gmd:abstract",NULL,NULL,NULL,NULL);?>
				<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:identificationInfo/".$md_srv."/gmd:purpose",NULL,NULL,NULL,NULL);?>

				<div  class="md_div md_inner_div">

				<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:identificationInfo/".$md_srv."/gmd:citation/gmd:CI_Citation/gmd:identifier/gmd:RS_Identifier","",array("/gmd:code","/gmd:codeSpace"),NULL,NULL);?>

				</div>

				<div id="md_identification_contact" class="md_div md_inner_div">

					<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:identificationInfo/".$md_srv."/gmd:pointOfContact/gmd:CI_ResponsibleParty","",array("/gmd:organisationName","/gmd:contactInfo/gmd:CI_Contact/gmd:phone/gmd:CI_Telephone/gmd:voice","/gmd:contactInfo/gmd:CI_Contact/gmd:phone/gmd:CI_Telephone/gmd:fascimile","/gmd:contactInfo/gmd:CI_Contact/gmd:address/gmd:CI_Address/gmd:electronicMailAddress","/gmd:role/gmd:CI_RoleCode"),NULL,NULL);?>
				...
				<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:identificationInfo/".$md_srv."/gmd:resourceConstraints/gmd:MD_Constraints/gmd:useLimitation",NULL,NULL,NULL,NULL);?>

				<?php

					$r=strtolower(strip_tags($gR->buildGMDHTML("//".$md_mi_srv."/gmd:identificationInfo/".$md_srv."/gmd:resourceConstraints/gmd:MD_LegalConstraints/gmd:accessConstraints/gmd:MD_RestrictionCode","","",true,NULL)));

					if ($r=="otherrestrictions")
					{
						echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:identificationInfo/".$md_srv."/gmd:resourceConstraints/gmd:MD_LegalConstraints/gmd:otherConstraints",NULL,NULL,NULL,NULL);
					}
					else
					{
						echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:identificationInfo/".$md_srv."/gmd:resourceConstraints/gmd:MD_LegalConstraints/gmd:accessConstraints/gmd:MD_RestrictionCode",NULL,NULL,NULL,NULL);
					}

				?>

				<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:identificationInfo/".$md_srv."/gmd:resourceConstraints/gmd:MD_SecurityConstraints/gmd:classification/gmd:MD_ClassificationCode",NULL,NULL,NULL,NULL);?>

				<div id="md_extend" class="md_div md_inner_div">
				<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:identificationInfo/".$md_srv."/".$md_extend."/gmd:EX_Extent/gmd:geographicElement","",array("/gmd:EX_GeographicBoundingBox/gmd:westBoundLongitude","/gmd:EX_GeographicBoundingBox/gmd:eastBoundLongitude","/gmd:EX_GeographicBoundingBox/gmd:southBoundLatitude","/gmd:EX_GeographicBoundingBox/gmd:northBoundLatitude"),NULL,NULL);?>
						...
						<div id="md_the_map">
							<script defer="defer" type="text/javascript">
								...
								<?php
									...
									$west=$gR->buildGMDHTML("//".$md_mi_srv."/gmd:identificationInfo/".$md_srv."/".$md_extend."/gmd:EX_Extent/gmd:geographicElement/gmd:EX_GeographicBoundingBox/gmd:westBoundLongitude","","",true,NULL);

									$east=$gR->buildGMDHTML("//".$md_mi_srv."/gmd:identificationInfo/".$md_srv."/".$md_extend."/gmd:EX_Extent/gmd:geographicElement/gmd:EX_GeographicBoundingBox/gmd:eastBoundLongitude","","",true,NULL);

									$south=$gR->buildGMDHTML("//".$md_mi_srv."/gmd:identificationInfo/".$md_srv."/".$md_extend."/gmd:EX_Extent/gmd:geographicElement/gmd:EX_GeographicBoundingBox/gmd:southBoundLatitude","","",true,NULL);

									$north=$gR->buildGMDHTML("//".$md_mi_srv."/gmd:identificationInfo/".$md_srv."/".$md_extend."/gmd:EX_Extent/gmd:geographicElement/gmd:EX_GeographicBoundingBox/gmd:northBoundLatitude","","",true,NULL);
				...
				<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:dataQualityInfo/gmd:DQ_DataQuality/gmd:lineage/gmd:LI_Lineage/gmd:statement",NULL,NULL,NULL,NULL);?>

				<?php

					$md_urls=$gR->buildGMDHTML("//".$md_mi_srv."/gmd:distributionInfo/gmd:MD_Distribution/gmd:transferOptions/gmd:MD_DigitalTransferOptions/gmd:onLine/gmd:CI_OnlineResource/gmd:linkage/gmd:URL",NULL,NULL,NULL,NULL);

					$srv_urls=$gR->buildGMDHTML("//".$md_mi_srv."/gmd:identificationInfo/srv:SV_ServiceIdentification/srv:containsOperations/srv:SV_OperationMetadata/srv:connectPoint/gmd:CI_OnlineResource/gmd:linkage",NULL,NULL,NULL,NULL);
				...
				<?php echo $gR->buildGMDHTML("//".$md_mi_srv."/gmd:dataQualityInfo/gmd:DQ_DataQuality/gmd:lineage/gmd:LI_Lineage/gmd:source/gmd:LI_Source","",array("/gmd:description","/gmd:scaleDenominator/gmd:MD_RepresentativeFraction/gmd:denominator"),NULL,NULL);?>
