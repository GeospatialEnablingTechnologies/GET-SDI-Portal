...
class validateWFSService
{
		...
		switch ($this->versionWFS)
		{
			...
			case "1.1.0":
				...
				if ($this->validateAgainstSchema($dom_xml,"1.1.0/wfs.xsd"))
				{
					...
					$pathToTransaction=$dom_xml->getElementsByTagNameNS("http://www.opengis.net/ows","OperationsMetadata")->item(0)->childNodes;
					
					$this->isValidWFST=$this->hasTransactionCapabilities($pathToTransaction,$dom_xml,$version);
			...
			case "1.0.0":
				...
				$pathToTransaction=$dom_xml->getElementsByTagName("Capability")->item(0)->getElementsByTagName("Request")->item(0)->childNodes;
	...
	function hasTransactionCapabilities($pathToTag,$dom_xml,$version)
	{
		$hasTransactionOperation="false";
		...
		foreach ($pathToTag as $key=>$value)
		{
			...
			if ($version="1.1.0")
			{
				$transaction=(string)$value->getAttribute("name");
			
				if (strtolower($transaction)=="transaction")
				{
					$hasTransactionOperation="true";
				}
			}
			if ($version="1.1.0")
			{
				$transaction=(string)$value->tagName;
		...
		return $hasTransactionOperation;
	}
